  ...
  convertSignature: (signature, keyType) => {
    ...
    switch (keyType) {
      case 'ssh-dss': {
        ...
        bufferCopy(r, signature, rOffset, r.length, 0);
        bufferCopy(s, signature, sOffset, s.length, r.length - rOffset);
        break;
      }
      case 'ecdsa-sha2-nistp256':
      case 'ecdsa-sha2-nistp384':
      case 'ecdsa-sha2-nistp521': {
        ...
        if (r === null || s === null)
          return;
        signature = Buffer.allocUnsafe(4 + r.length + 4 + s.length);
        writeUInt32BE(signature, r.length, 0);
        signature.set(r, 4);
        writeUInt32BE(signature, s.length, 4 + r.length);
        signature.set(s, 4 + 4 + r.length);
        break;
      }
    }
    return {
